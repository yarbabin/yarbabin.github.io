...
import { ParticipantStatsModal, ParticipantNameButton } from '../components/ParticipantStatsModal';

export default function Analytics() {
  const [data, setData] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const [selectedParticipant, setSelectedParticipant] = useState<{ id: string; cupId?: string } | null>(null);
  ...
  const openParticipant = (id?: string, cupId?: string) => {
    if (id) setSelectedParticipant({ id, cupId });
  };
    ...
    <div>
      ...
      <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-6 ${loading ? 'opacity-50 pointer-events-none' : 'transition-opacity duration-300'}`}>
            ...
            <div className="bg-white border-4 border-black p-4 md:p-6 shadow-[4px_4px_0px_0px_#000]">
              <div className="flex flex-col md:flex-row md:items-baseline gap-2 md:gap-4 mb-4 md:mb-6">
                ...
                <ParticipantNameButton
                  name={data.bestResult.participant_name}
                  onClick={() => openParticipant(data.bestResult.participant_id, data.bestResult.cup_id)}
                  className="text-xl md:text-3xl bg-secondary border-2 border-black"
                />
        ...
        <div className="brutal-card p-4 md:p-6 bg-success">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 md:gap-3 mb-4 md:mb-6 bg-white inline-flex px-3 py-2 md:px-4 border-4 border-black shadow-[4px_4px_0px_0px_#000]">
            ...
            <span className="text-xs md:text-sm font-bold">(от 5 туров)</span>
          </div>
          {data.bestAverage ? (
            <div className="bg-white border-4 border-black p-4 md:p-6 shadow-[4px_4px_0px_0px_#000]">
              ...
              <ParticipantNameButton
                name={data.bestAverage.participant_name}
                onClick={() => openParticipant(data.bestAverage.participant_id)}
                className="text-lg md:text-2xl mb-2 bg-secondary border-2 border-black"
              />
              <div className="text-sm md:text-lg font-bold text-black">Сыграно туров: {data.bestAverage.games_played}</div>
            </div>
          ) : (
            <p className="font-bold text-base md:text-lg bg-white p-4 border-4 border-black inline-block">Нет данных (нужно минимум 5 туров)</p>
          )}
        </div>
        ...
        <div className="brutal-card p-4 md:p-6 bg-[#cb99ff]">
          ...
          {data.closestDistance ? (
            <div className="bg-white border-4 border-black p-4 md:p-6 shadow-[4px_4px_0px_0px_#000]">
              ...
              <ParticipantNameButton
                name={data.closestDistance.participant_name}
                onClick={() => openParticipant(data.closestDistance.participant_id, data.closestDistance.cup_id)}
                className="text-lg md:text-2xl mb-2 bg-secondary border-2 border-black"
              />
        ...
        <div className="brutal-card p-4 md:p-6 bg-warning">
          ...
          {data.bestAvgYears ? (
            <div className="bg-white border-4 border-black p-4 md:p-6 shadow-[4px_4px_0px_0px_#000]">
              ...
              <ParticipantNameButton
                name={data.bestAvgYears.participant_name}
                onClick={() => openParticipant(data.bestAvgYears.participant_id)}
                className="text-lg md:text-2xl mb-2 bg-secondary border-2 border-black"
              />
              <div className="text-sm md:text-lg font-bold text-black">Сыграно туров: {data.bestAvgYears.games_played}</div>
            </div>
          ) : (
            <p className="font-bold text-base md:text-lg bg-white p-4 border-4 border-black inline-block">Нет данных (нужно минимум 5 туров)</p>
          )}
        ...
        <div className="brutal-card p-4 md:p-6 bg-accent">
          ...
          {data.bestAvgDistance ? (
            <div className="bg-white border-4 border-black p-4 md:p-6 shadow-[4px_4px_0px_0px_#000]">
              ...
              <ParticipantNameButton
                name={data.bestAvgDistance.participant_name}
                onClick={() => openParticipant(data.bestAvgDistance.participant_id)}
                className="text-lg md:text-2xl mb-2 bg-secondary border-2 border-black"
              />
              <div className="text-sm md:text-lg font-bold text-black">Сыграно туров: {data.bestAvgDistance.games_played}</div>
            </div>
          ) : (
            <p className="font-bold text-base md:text-lg bg-white p-4 border-4 border-black inline-block">Нет данных (нужно минимум 5 туров)</p>
          )}
      ...
      {selectedParticipant && (
        <ParticipantStatsModal
          ...
          cupId={selectedParticipant.cupId}
